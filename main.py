...
for result in yolo_results[0].boxes:
    x1, y1, x2, y2 = map(int, result.xyxy[0].tolist())  # Corrected `map` usage
    conf = result.conf.item()  # Extract confidence score
    cls_idx = int(result.cls.item())  # Extract class index
    label = yolo_model.names[cls_idx]  # Map class index to name
